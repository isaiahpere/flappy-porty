  ...
  return (
    <article
      className="w-full flex items-center justify-between rounded-3xl 
        ...
        lg:flex-col lg:p-8 xs:rounded-2xl xs:rounded-br-3 xs:p-4xl"
    >
      ...
      <div className="w-1/2 flex flex-col items-start justify-between pl-6 lg:w-full lg:pl-0 lg:pt-6">
        <span className="text-primary font-medium text-3xl mb-2 dark:text-primaryDark xs:text-base">
          {type}
        </span>
        <Link href={link} className="hover:underline underline-offset-2">
          <h2 className="my-4 w-full text-left text-4xl font-bold dark:text-light sm:text-sm">
            {title}
          </h2>
        </Link>
        <p className="my-4 font-medium text-dark dark:text-light sm:text-sm">
          {summary}
        </p>
        <div className="my-z flex items-center">
          ...
          <Link
            ...
            target="_blank"
            className="
              ml-4 rounded-lg bg-dark border border-dark text-light p-2 px-6 text-lg font-semibold 
...
const Project = ({ title, type, image, link, githubLink }) => {
  return (
    <article className="w-full flex flex-col item-center justify-center rounded-2xl border border-solid border-dark bg-light p-6 relative dark:bg-dark dark:border-light xs:p-4">
      <div className="absolute top-0 -right-3 -z-10 w-[101%] h-[103%] rounded-[2rem] bg-dark rounded-br-3xl dark:bg-light md:-right-2 md:w-[101]% xs:h-[102%] xs:rounded-[1.5]rem" />
      <Link
        href={link}
        target="_blank"
      ...
      <div className="w-full flex flex-col items-start justify-between mt-4">
        <span className="text-primary  mb-2 dark:text-primaryDark lg:text-lg md:text-base">
          {type}
        </span>
        <Link href={link} className="hover:underline underline-offset-2">
      ...
      <main className="w-full mb-16 flex flex-col items-center justify-center dark:text-light">
        <Layout className="pt-16">
          <AnimatedText
            ...
            className="text-8xl mb-16 lg:text-7xl sm:mb-8 sx:text-6xl xs:text-4xl"
          />
